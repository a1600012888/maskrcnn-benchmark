...
class DomainCocoDetection(torchvision.datasets.coco.CocoDetection):
    def __init__(self, ann_file, root, embedding_dir,
                 raw_embedding_dir, jitter_range):
        ...
        self.raw_embedding_dir = raw_embedding_dir
        self.jitter_range = jitter_range
        with open(os.path.join(self.raw_embedding_dir, 'name2domain.json'), 'r') as f:
            self.raw_name2domain = json.load(f)
    ...
    def __getitem__(self, index):
        ...
        raw_vec_idx = self.raw_name2domain[file_name]
...
class EmbedJitterDomainDataset(DomainCocoDetection):
    def __init__(
        self, ann_file, root, embedding_dir, raw_embedding_dir,
            jitter_range, remove_images_without_annotations, transforms=None
    ):
        super(EmbedJitterDomainDataset, self).__init__(ann_file, root, embedding_dir,
                                                       raw_embedding_dir, jitter_range)
